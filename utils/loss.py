...
class ComputeLoss:
    ...
    def checkTargetsFeatureMap(self, tobj, paths, targets):
        #note : batch size must be one
        import cv2
        from .rboxs_utils import rbox2poly
        from .plots import Annotator, Colors
        cls_names = ['plane', 'baseball-diamond', 'bridge', 'ground-track-field', 'small-vehicle',
                     'large-vehicle', 'ship', 'tennis-court', 'basketball-court', 'storage-tank',
                     'soccer-ball-field', 'roundabout', 'harbor', 'swimming-pool', 'helicopter',
                     'container-crane']
        expriments_root = 'D:\workspace\python_workspace\yolov5-obb-detection\expriments'
        img = cv2.imread(paths[0])
        img_name = paths[0].split('\\')[-1]
        tobj_view = tobj.clone().mul_(255).detach().type(torch.uint8).cpu().permute(0,2,3,1).numpy()
        tbox_view = targets[:, 2: 7].clone().detach().cpu().numpy()
        label_view = targets[:, 1].clone().detach().cpu().numpy()
        ih, iw, ic = img.shape
        bs, h, w, c = tobj_view.shape #c denote to anchor
        stride = int(iw / w)
        if stride == 8:
            color = (3, 88, 220)
        elif stride == 16:
        ...
        for i in range(h):
            for j in range(w):
                ...
                judge = (int(tobj_view[0, i, j, 0]), int(tobj_view[0, i, j, 1]), int(tobj_view[0, i, j, 2]))
                if judge != (0, 0, 0):
                    #print(tobj[0, :, i, j])
                    cv2.circle(img, (x, y), 2, color, 2)

        #for rbox in tbox_view:
        polys = rbox2poly(tbox_view)
        annotator = Annotator(img, line_width=2, example=str(cls_names))
        colors = Colors()
        ...
        cv2.imwrite(os.path.join(expriments_root, 'stride_'+str(stride), img_name), img)
